...
test_that("test firefox_ts_to_posix()", {
  expect_equal(firefox_ts_to_posix(ff_ts), ts)
  expect_equal(firefox_ts_to_posix(ff_ts, tz = "CET"), with_tz(ts, tz = "CET"))
...
test_that("test posix_to_firefox_ts()", {
  ...
  expect_equal(posix_to_firefox_ts(with_tz(ts, tz = "CET")), ff_ts)
